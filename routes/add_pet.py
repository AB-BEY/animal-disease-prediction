...
async def get_all_pets(current_user_id: int = Depends(get_current_user_id),db: Session = Depends(get_db)):
    all_pets = db.exec(select(Animal).where(Animal.user_id == current_user_id)).all()
    return all_pets